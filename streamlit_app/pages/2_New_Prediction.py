...
with col2:
    ...
    mmse = st.number_input(
        ...
        min_value=0, max_value=30, value=28, step=1,
    )
    cdr = st.select_slider(
    ...
    educ = st.number_input(
        ...
        min_value=4, max_value=23, value=16, step=1,
    )
    nwbv = st.number_input(
        "Normalized Whole Brain Volume (nWBV)",
        min_value=0.6, max_value=0.85, value=0.75,
        step=0.01, format="%.3f",
    )
    etiv = st.number_input(
        "Estimated Total Intracranial Volume (eTIV)",
        min_value=1100, max_value=2000, value=1500, step=1,
    )
    asf = st.number_input(
        "Atlas Scaling Factor (ASF)",
        min_value=0.8, max_value=1.6, value=1.2,
        step=0.01, format="%.2f",
    )
...
if submit:
    if uploaded_file is None:
        ...
        with st.spinner("Analyzing data..."):
            ...
            features = [
                float(mmse), float(cdr), float(age), float(educ),
                float(nwbv), float(etiv), float(asf), float(ses), float(gender),
            ]
            feature_names = [
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    with xai_col1:
                        gradcam = result.get("explainability", {}).get("gradcam")
                        if gradcam:
                    ...
                    with xai_col2:
                        shap_result = result.get("explainability", {}).get("shap")
                        if shap_result:
                            render_shap(shap_result)
                        else:
